...
function upload(client, readableStream, remoteFilename) {
    ...
    return client.handle(command, (res, task) => {
        if (res.code === 150) { // Ready to upload
            ...
            client.dataSocket.once("finish", () => client.dataSocket = undefined);
 ...
 * 
 * @param {Client} client 
 * @param {string} remoteDirName 
 */
async function removeDir(client, remoteDirName) {
    await send(client, "CWD " + remoteDirName);
    ...
    if (workingDir !== "/") {
        ...
        await send(client, "RMD " + remoteDirName);        
    }
}
...
/**
 * Create a directory at the given path and all intermediate directories if necessary.
 * 
 * @param {Client} client 
 * @param {string} path 
 */
async function ensureDir(client, path) {
    ...
    // If the path was absolute go to root directory.
    if (path.startsWith("/")) {
        await cd(client, "/");
    }
