...
/**
 * Parse an FTP response into response groups. A response group is a single- or
 * complete multiline response. An FTP response can also contain multiple 
 * multiline responses that will each be held by a response group. A response can 
 * also be incomplete (and be completed on the next incoming data chunk) for which
 * case this function also describes a `rest`. This will also convert all CRLF into LF.
 * 
 * @param {string} text 
