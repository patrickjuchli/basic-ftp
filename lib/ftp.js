...
const uploadDirFileFilter = require("./uploadDirFileFilter");
...
class Client {
     ...
     * @param {(filename: string) => boolean} [filterByName]  A filter function that receives a filename and is expected to return true if the file name is accepted.
     */
    async uploadDir(localDirPath, remoteDirName = undefined, filterByName = uploadDirFileFilter) {
        ...
        await uploadDirContents(this, localDirPath, filterByName);
 ...
 * @param {Client} client
 * @param {string} localDirPath 
 * @param {(filename: string) => boolean} filterByName
 */
async function uploadDirContents(client, localDirPath, filterByName) {
    const files = await fsReadDir(localDirPath);
    const filtered = files.filter(filterByName);
    for (const file of filtered) {
        ...
        if (stats.isFile()) {
            ...
            await uploadDirContents(client, fullPath, filterByName);
