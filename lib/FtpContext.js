...
exports.FTPContext = class FTPContext {
    ...
    _setupErrorHandlers(socket, identifier) {
        // Start timing how long the connection is open.  `connect()` may or may
        // not have been called yet, so we initialize it, but then reset if
        // there's a `connect` event
        let start = Date.now();
        socket.once("connect", () => start = Date.now());
        ...
        socket.once("timeout", () => {
            this._closeWithError(new Error(`Timeout after ${(Date.now() - start)/1000} seconds (${identifier})`));
        });
    }
