...
module.exports = class FTPContext {  
    ...
    handle(command, handler) {
        handler._command = command;  // So we can reference the command name later if there's an issue
        if (this._handler !== undefined) {
            this.close();
            throw new Error(`"${this._handler._command}" is still active.  Use await or .then() before running other commands.`);
        }
