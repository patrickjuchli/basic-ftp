...
module.exports = class FTPContext {  
    ...
    set socket(socket) {
        ...
        if (socket) {
            ...
            socket.once("close", hadError => hadError ? this._closeWithError("Control socket closed due to transmission error.") : undefined);
        }
        else {
            this._closeSocket(this._socket);
        }
    ...
    set dataSocket(socket) {
        ...
        if (socket) {
            ...
            socket.once("close", hadError => hadError ? this._closeWithError("Data socket closed due to transmission error,") : undefined);
        }
